...
def to_local(dt, tz_str):
    """Converteer een (UTC) datetime naar de lokale tijdzone van het opgegeven IANA-tz string.
    Geeft het origineel terug als omzetten niet kan (bv. naive datetime of onbekende tz)."""
    if dt is None or pd.isnull(dt) or not tz_str:
        return dt
    try:
        if getattr(dt, 'tzinfo', None) is None:
            dt = dt.tz_localize('UTC')
        return dt.tz_convert(tz_str)
    except Exception:
        return dt
    ...
    try:
        ...
        if not df_clean.empty:
            ...
            if st.session_state.get('run_main') and fa_api_key:
                ...
                if actual_registration and actual_registration.upper() != csv_registration.upper():
                    st.warning(
                        f"⚠️ De CSV koppelt {flight_display_name} op {search_date.strftime('%d-%m-%Y')} aan **{csv_registration}**, "
                        f"maar volgens FlightAware werd deze vlucht uitgevoerd door **{actual_registration}**. "
                        f"De rotatie-analyse gebruikt **{actual_registration}** zodat de getoonde rotatie kloppend is."
                    )
                ...
                filtered_target = [f for f in (target_flight_data or [])
                                   if f and (not f.get('registration')
                                             or str(f.get('registration')).strip().upper() == registration.upper())]

                combined_flights = reg_flights + filtered_target
                unique_flights = []
                seen_keys = set()
                ...
                if unique_flights:
                    ...
                    sorted_all = sorted(unique_flights, key=lambda x: pd.to_datetime(x.get('scheduled_out')))

                    # FIX: Bouw een LOGISCHE ROTATIE-KETEN rondom de gezochte vlucht.
                    # De FlightAware-API kan vluchten teruggeven uit andere operationele cycli
                    # van hetzelfde toestel die toevallig binnen het 48u-venster vallen
                    # (bv. een AMS-MSP-AMS rondvlucht op een ándere dag dan DL056). Door de
                    # keten op te bouwen vanuit de anker-vlucht en alleen aansluitende
                    # vluchten toe te voegen (origin = vorige destination EN tijdvolgorde
                    # consistent), filteren we deze artefacten eruit.
                    def _origin_code(f):
                        o = f.get('origin') or {}
                        return o.get('code_iata') or o.get('code_icao')
                    def _dest_code(f):
                        d = f.get('destination') or {}
                        return d.get('code_iata') or d.get('code_icao')
                    def _std_utc(f):
                        return pd.to_datetime(f.get('scheduled_out'))
                    def _sta_utc(f):
                    ...
                    if anchor_idx is not None:
                        chain = [sorted_all[anchor_idx]]
                        anchor_std_utc_date = _std_utc(sorted_all[anchor_idx]).date()
                        # Beperking: alleen vluchten meenemen die op DEZELFDE UTC-vertrekdag
                        # als het anker vertrekken. Een vlucht mag wel pas de dag erna landen.
                        # Vluchten die de volgende UTC-dag vertrekken (zoals een MSP-AMS om
                        # 00:50 UTC) horen bij een andere operationele dag-cyclus.
                        # Achteruit: voeg vluchten toe waarvan destination aansluit op huidige origin
                        # en STA <= STD van huidige (geen tijd-overlap).
                        current = sorted_all[anchor_idx]
                        for j in range(anchor_idx - 1, -1, -1):
                            cand = sorted_all[j]
                            if _std_utc(cand).date() != anchor_std_utc_date:
                                break  # gesorteerd op tijd; alles eerder zit op andere dag
                            if _dest_code(cand) and _origin_code(current) and \
                               _dest_code(cand) == _origin_code(current) and \
                               pd.notnull(_sta_utc(cand)) and pd.notnull(_std_utc(current)) and \
                               _sta_utc(cand) <= _std_utc(current):
                                chain.insert(0, cand); current = cand
                        # Vooruit: voeg vluchten toe waarvan origin aansluit op huidige destination
                        # en STD >= STA van huidige.
                        current = sorted_all[anchor_idx]
                        for j in range(anchor_idx + 1, len(sorted_all)):
                            cand = sorted_all[j]
                            if _std_utc(cand).date() != anchor_std_utc_date:
                                break  # gesorteerd op tijd; alles later zit op andere dag
                            if _origin_code(cand) and _dest_code(current) and \
                               _origin_code(cand) == _dest_code(current) and \
                               pd.notnull(_std_utc(cand)) and pd.notnull(_sta_utc(current)) and \
                               _std_utc(cand) >= _sta_utc(current):
                    ...
                    for f in sorted_flights:
                        ...
                        # Tijdzones uit FlightAware (IANA strings, bv. "America/Denver", "Europe/Amsterdam")
                        origin_tz = f.get('origin', {}).get('timezone')
                        dest_tz = f.get('destination', {}).get('timezone')

                        # Lokale tijden voor weergave (de keten-filter heeft al bepaald welke vluchten erbij horen).
                        std_origin_local = to_local(std, origin_tz)
                        ...
                        # Berekeningen blijven in UTC (verschillen kloppen ook tussen tijdzones).
                        ...
                        # FIX: Voor weergave omzetten naar lokale tijdzone van origin (STD/ATD)
                        # en destination (STA/ATA), zodat tijden overeenkomen met het airport-systeem.
                        std_disp = std_origin_local
                        atd_disp = to_local(atd_val, origin_tz)
                        sta_disp = to_local(sta, dest_tz)
                        ata_disp = to_local(ata_val, dest_tz)
                        ...
                        rows.append({
                            ...
                            'Slot Out (STD)': format_time(std_disp), 'Actual Out (ATD)': format_time(atd_disp), 'Dep Delay': f"{dep_delay}m",
                            'Actual In (ATA)': format_time(ata_disp), 'Slot In (STA)': format_time(sta_disp), 'Arr Delay': f"{arr_delay}m",
